...
import { addExternalInspection, deleteExternalInspection, getExternalInspectionByLaporanInspection, updateExternalInspection } from "../../services/External_inspection.service";
...
const ExternalInspection = () => {
  const { id } = useParams();
  const [ExternalInspection, setExternalInspection] = useState([]);
  ...
  useEffect(() => {
    fetchExternalInspection();
    fetchHistoricalMemorandum();
  }, []);
  

  const fetchExternalInspection = async () => {
    try {
      setLoading(true);
      const data = await getExternalInspectionByLaporanInspection(id);
      setExternalInspection(data.data);
      console.log("External Inspection Data:", data.data);
      
      // Fetch data logic here
      // setExternalInspection(data);
    } catch (error) {
      console.error("Error fetching External inspection data:", error);
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleAddExternalInspection = async (e) => {
    e.preventDefault();
    try {
      ...
      const data = await addExternalInspection(formData);
      console.log("Added External Inspection:", data);
      handleClose();
      fetchExternalInspection();
      Swal.fire({
        title: 'Berhasil!',
        text: 'External Inspection berhasil ditambahkan!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error adding External inspection:", error);
      setValidation(error.response?.data.errors || []);
    } finally {
      setIsSubmitting(false);
    }
  }
  const handleUpdateExternalInspection = async (e) => {
    e.preventDefault();
    try {
      setIsSubmitting(true);
      const formData = new FormData(e.target);
      ...
      formData.append('laporan_inspection_id', id);
      const data = await updateExternalInspection(SelectedEditData.id, formData);
      console.log("updated External Inspection:", data);
      handleClose();
      fetchExternalInspection();
      Swal.fire({
        title: 'Berhasil!',
        text: 'External Inspection berhasil diupdate!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error updating External inspection:", error);
      setValidation(error.response?.data.errors || []);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleDelete = async (row) => {
      const result = await Swal.fire({
          ...
          text: "Data External Inspection akan dihapus secara permanen!",
          icon: "warning",
      ...
      if (result.isConfirmed) {
          try {
              const res = await deleteExternalInspection(row.id);
              if (res.success) {
                  Swal.fire("Berhasil!", "External Inspection berhasil dihapus!", "success");
                  fetchExternalInspection();
              } else {
                  Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus External Inspection!", "error");
              }
          } catch (error) {
              console.error("Error deleting External Inspection:", error);
              Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus External Inspection!", "error");
          }
      }
  };
  ...
  return (
    <div className="flex flex-col space-y-4">
      <div className="flex flex-row justify-between space-x-4">
        <h1 className="text-xl font-bold p-2 uppercase">External Inspection</h1>
        <div className="flex justify-end gap-2 items-center">
          ...
          <button
            onClick={fetchExternalInspection}
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleAddExternalInspection(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
              Tambah External Inspection
            </h1>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleUpdateExternalInspection(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
