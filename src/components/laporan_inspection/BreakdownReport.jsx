...
import { addBreakdownReport, deleteBreakdownReport, getBreakdownReportByLaporanInspection, updateBreakdownReport } from "../../services/breakdown_report.service";
...
const BreakdownReport = () => {
  ...
  const fetchBreakdownReport = async () => {
    try {
      setLoading(true);
      const data = await getBreakdownReportByLaporanInspection(id);
      setBreakdownReport(data.data);
      console.log("Breakdown Report Data:", data.data);
      
      // Fetch data logic here
      // setBreakdownReport(data);
    } catch (error) {
      console.error("Error fetching Breakdown Report data:", error);
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleAddBreakdownReport = async (e) => {
    e.preventDefault();
    try {
      ...
      const data = await addBreakdownReport(formData);
      console.log("Added Breakdown Report:", data);
      handleClose();
      fetchBreakdownReport();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Breakdown Report berhasil ditambahkan!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error adding Breakdown Report:", error);
      setValidation(error.response?.data.errors || []);
    } finally {
      setIsSubmitting(false);
    }
  }
  const handleUpdateBreakdownReport = async (e) => {
    e.preventDefault();
    try {
      setIsSubmitting(true);
      const formData = new FormData(e.target);
      ...
      formData.append('laporan_inspection_id', id);
      const data = await updateBreakdownReport(SelectedEditData.id, formData);
      console.log("updated Breakdown Report:", data);
      handleClose();
      fetchBreakdownReport();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Breakdown Report berhasil diupdate!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
  ...
  const columns = [
    ...
    { field: 'breakdown_report_date', headerName: 'Tanggal Report', width: 150 },
      ...
      <div className='flex py-2 pl-5 '>
        {params.row.laporan_file ? (
          <Tooltip title={`${params.row.laporan_file}`} placement='bottom'>
            <Link
              to={`${base_public_url}laporan_inspection/breakdown_report/${params.value}`}
              target='_blank'
              className='text-lime-400 px-2 rounded-md hover:underline cursor-pointer'
  ...
  const handleDelete = async (row) => {
      const result = await Swal.fire({
          ...
          text: "Data Breakdown Report akan dihapus secara permanen!",
          icon: "warning",
      ...
      if (result.isConfirmed) {
          try {
              const res = await deleteBreakdownReport(row.id);
              if (res.success) {
                  Swal.fire("Berhasil!", "Breakdown Report berhasil dihapus!", "success");
                  fetchBreakdownReport();
              } else {
                  Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Breakdown Report!", "error");
              }
          } catch (error) {
  ...
  return (
    <div className="flex flex-col space-y-4">
      <div className="flex flex-row justify-between space-x-4">
        <h1 className="text-xl font-bold p-2 uppercase">Breakdown Report</h1>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleAddBreakdownReport(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
              Tambah Breakdown Report
            </h1>
            <div className="space-y-3">
              ...
              <div>
                <label htmlFor="breakdown_report_date">Report Date<sup className='text-red-500'>*</sup></label>
                <input type="date" name="breakdown_report_date" className="border rounded-md p-2 w-full" required />
              </div>
              <div>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleUpdateBreakdownReport(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label htmlFor="breakdown_report_date">Report Date<sup className='text-red-500'>*</sup></label>
                <input type="date" name="breakdown_report_date" defaultValue={SelectedEditData.breakdown_report_date} className="border rounded-md p-2 w-full" required />
              </div>
              <div>
                <label htmlFor="breakdown_report_date">Ada Memo ?</label>
              ...
              <div>
                ...
                <input 
                  ...
                  <div className="text-sm bg-lime-300 text-emerald-950 p-2 rounded-xl">
                    <Link className="hover:underline" target="_blank" onClick={() => handleAddActivity(SelectedEditData?.laporan_file, "LAPORAN INSPEKSI")} to={`${base_public_url}laporan_inspection/breakdown_report/${SelectedEditData.laporan_file}`}>{SelectedEditData.laporan_file ? SelectedEditData.laporan_file : '-'}</Link>
                  </div>
              </div>
              }
            </div>
