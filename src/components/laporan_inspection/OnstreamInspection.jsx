...
import { addOnstreamInspection, deleteOnstreamInspection, getOnstreamInspectionByLaporanInspection, updateOnstreamInspection } from "../../services/onstream_inspection.service";
...
const OnstreamInspection = () => {
  const { id } = useParams();
  const [OnstreamInspection, setOnstreamInspection] = useState([]);
  ...
  useEffect(() => {
    fetchOnstreamInspection();
    fetchHistoricalMemorandum();
  }, []);
  

  const fetchOnstreamInspection = async () => {
    try {
      setLoading(true);
      const data = await getOnstreamInspectionByLaporanInspection(id);
      setOnstreamInspection(data.data);
      console.log("Onstream Inspection Data:", data.data);
      
      // Fetch data logic here
      // setOnstreamInspection(data);
    } catch (error) {
      console.error("Error fetching Onstream inspection data:", error);
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleAddOnstreamInspection = async (e) => {
    e.preventDefault();
    try {
      ...
      const data = await addOnstreamInspection(formData);
      console.log("Added Onstream Inspection:", data);
      handleClose();
      fetchOnstreamInspection();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Onstream Inspection berhasil ditambahkan!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error adding Onstream inspection:", error);
      setValidation(error.response?.data.errors || []);
    } finally {
      setIsSubmitting(false);
    }
  }
  const handleUpdateOnstreamInspection = async (e) => {
    e.preventDefault();
    try {
      setIsSubmitting(true);
      const formData = new FormData(e.target);
      ...
      formData.append('laporan_inspection_id', id);
      const data = await updateOnstreamInspection(SelectedEditData.id, formData);
      console.log("updated Onstream Inspection:", data);
      handleClose();
      fetchOnstreamInspection();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Onstream Inspection berhasil diupdate!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error updating Onstream inspection:", error);
      setValidation(error.response?.data.errors || []);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const columns = [
    ...
    { field: 'inspection_date', headerName: 'Tanggal Inspeksi', width: 150 },
      ...
      <div className='flex py-2 pl-5 '>
        {params.row.laporan_file ? (
          <Tooltip title={`${params.row.laporan_file}`} placement='bottom'>
            <Link
              to={`${base_public_url}laporan_inspection/onstream_inspection/${params.value}`}
              target='_blank'
              className='text-lime-400 px-2 rounded-md hover:underline cursor-pointer'
  ...
  const handleDelete = async (row) => {
      const result = await Swal.fire({
          ...
          text: "Data Onstream Inspection akan dihapus secara permanen!",
          icon: "warning",
      ...
      if (result.isConfirmed) {
          try {
              const res = await deleteOnstreamInspection(row.id);
              if (res.success) {
                  Swal.fire("Berhasil!", "Onstream Inspection berhasil dihapus!", "success");
                  fetchOnstreamInspection();
              } else {
                  Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Onstream Inspection!", "error");
              }
          } catch (error) {
              console.error("Error deleting Onstream Inspection:", error);
              Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Onstream Inspection!", "error");
          }
      }
  };
  ...
  return (
    <div className="flex flex-col space-y-4">
      <div className="flex flex-row justify-between space-x-4">
        <h1 className="text-xl font-bold p-2 uppercase">Onstream Inspection</h1>
        <div className="flex justify-end gap-2 items-center">
          ...
          <button
            onClick={fetchOnstreamInspection}
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleAddOnstreamInspection(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
              Tambah Onstream Inspection
            </h1>
            <div className="space-y-3">
              ...
              <div>
                <label htmlFor="inspection_date">Inspection Date<sup className='text-red-500'>*</sup></label>
                <input type="date" name="inspection_date" className="border rounded-md p-2 w-full" required />
              </div>
              <div>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleUpdateOnstreamInspection(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
              Update Onstream Inspection
            </h1>
            <div className="space-y-3">
              <div>
                <label htmlFor="judul">Judul<sup className='text-red-500'>*</sup> </label>
              ...
              <div>
                <label htmlFor="inspection_date">Inspection Date<sup className='text-red-500'>*</sup></label>
                <input type="date" name="inspection_date" defaultValue={SelectedEditData.inspection_date} className="border rounded-md p-2 w-full" required />
              </div>
              <div>
                <label htmlFor="inspection_date">Ada Memo ?</label>
              ...
              <div>
                ...
                <input 
                  ...
                  <div className="text-sm bg-lime-300 text-emerald-950 p-2 rounded-xl">
                    <Link className="hover:underline" target="_blank" onClick={() => handleAddActivity(SelectedEditData?.laporan_file, "LAPORAN INSPEKSI") } to={`${base_public_url}laporan_inspection/onstream_inspection/${SelectedEditData.laporan_file}`}>{SelectedEditData.laporan_file ? SelectedEditData.laporan_file : '-'}</Link>
                  </div>
              </div>
              }
            </div>
...
export default OnstreamInspection