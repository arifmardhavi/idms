...
import { addPreventive, deletePreventive, getPreventiveByLaporanInspection, updatePreventive } from "../../services/preventive.service";
...
const Preventive = () => {
  ...
  const fetchPreventive = async () => {
    try {
      setLoading(true);
      const data = await getPreventiveByLaporanInspection(id);
      setPreventive(data.data);
      console.log("Preventive Data:", data.data);
  ...
  const handleAddPreventive = async (e) => {
    e.preventDefault();
    try {
      ...
      const data = await addPreventive(formData);
      console.log("Added Preventive:", data);
      handleClose();
      fetchPreventive();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Preventive berhasil ditambahkan!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error adding Preventive:", error);
      setValidation(error.response?.data.errors || []);
    } finally {
      setIsSubmitting(false);
    }
  }
  const handleUpdatePreventive = async (e) => {
    e.preventDefault();
    try {
      setIsSubmitting(true);
      const formData = new FormData(e.target);
      ...
      formData.append('laporan_inspection_id', id);
      const data = await updatePreventive(SelectedEditData.id, formData);
      console.log("updated Preventive:", data);
      handleClose();
      fetchPreventive();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Preventive berhasil diupdate!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
  ...
  const columns = [
    ...
    { field: 'preventive_date', headerName: 'Tanggal Preventive', width: 220 },
      ...
      <div className='flex py-2 pl-5 '>
        {params.row.laporan_file ? (
          <Tooltip title={`${params.row.laporan_file}`} placement='bottom'>
            <Link
              to={`${base_public_url}laporan_inspection/preventive/${params.value}`}
              target='_blank'
              className='text-lime-400 px-2 rounded-md hover:underline cursor-pointer'
  ...
  const handleDelete = async (row) => {
      const result = await Swal.fire({
          ...
          text: "Data Preventive akan dihapus secara permanen!",
          icon: "warning",
      ...
      if (result.isConfirmed) {
          try {
              const res = await deletePreventive(row.id);
              if (res.success) {
                  Swal.fire("Berhasil!", "Preventive berhasil dihapus!", "success");
                  fetchPreventive();
              } else {
                  Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Preventive!", "error");
              }
          } catch (error) {
  ...
  return (
    <div className="flex flex-col space-y-4">
      <div className="flex flex-row justify-between space-x-4">
        <h1 className="text-xl font-bold p-2 uppercase">Preventive Maint</h1>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleAddPreventive(e)} method="POST" className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label htmlFor="preventive_date">Preventive Date<sup className='text-red-500'>*</sup></label>
                <input type="date" name="preventive_date" className="border rounded-md p-2 w-full" required />
              </div>
              <div>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleUpdatePreventive(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
              Update Preventive
            </h1>
            <div className="space-y-3">
              ...
              <div>
                <label htmlFor="preventive_date">Preventive Date<sup className='text-red-500'>*</sup></label>
                <input type="date" name="preventive_date" defaultValue={SelectedEditData.preventive_date} className="border rounded-md p-2 w-full" required />
              </div>
              <div>
                <label htmlFor="preventive_date">Ada Memo ?</label>
              ...
              <div>
                ...
                <input 
                  ...
                  <div className="text-sm bg-lime-300 text-emerald-950 p-2 rounded-xl">
                    <Link className="hover:underline" target="_blank" onClick={() => handleAddActivity(SelectedEditData?.laporan_file, "LAPORAN INSPEKSI") } to={`${base_public_url}laporan_inspection/preventive/${SelectedEditData.laporan_file}`}>{SelectedEditData.laporan_file ? SelectedEditData.laporan_file : '-'}</Link>
                  </div>
              </div>
              }
            </div>
