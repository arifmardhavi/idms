...
const Surveillance = () => {
  ...
  const handleAddSurveillance = async (e) => {
    ...
    try {
      ...
      console.log("Added Surveillance:", data);
      handleClose();
      fetchSurveillance();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Surveillance berhasil ditambahkan!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error adding Surveillance:", error);
  ...
  const handleUpdateSurveillance = async (e) => {
    ...
    try {
      ...
      console.log("updated Surveillance:", data);
      handleClose();
      fetchSurveillance();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Surveillance berhasil diupdate!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
  ...
  const columns = [
    ...
    { field: 'surveillance_date', headerName: 'Tanggal Surveillance', width: 150 },
  ...
  const handleDelete = async (row) => {
      const result = await Swal.fire({
          ...
          text: "Data Surveillance akan dihapus secara permanen!",
          icon: "warning",
      ...
      if (result.isConfirmed) {
          try {
              ...
              if (res.success) {
                  Swal.fire("Berhasil!", "Surveillance berhasil dihapus!", "success");
                  fetchSurveillance();
              } else {
                  Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Surveillance!", "error");
              }
          } catch (error) {
              console.error("Error deleting Surveillance:", error);
              Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Surveillance!", "error");
          }
      }
  };
  ...
  return (
    <div className="flex flex-col space-y-4">
      <div className="flex flex-row justify-between space-x-4">
        <h1 className="text-xl font-bold p-2 uppercase">Surveillance</h1>
        ...
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleAddSurveillance(e)} method="POST" className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label htmlFor="surveillance_date">Surveillance Date<sup className='text-red-500'>*</sup></label>
              ...
              <div>
                ...
                <input 
                  ...
                  <div className="text-sm bg-lime-300 text-emerald-950 p-2 rounded-xl">
                    <Link className="hover:underline" target="_blank" to={`${base_public_url}laporan_inspection/surveillance/${SelectedEditData.laporan_file}`}>{SelectedEditData.laporan_file ? SelectedEditData.laporan_file : '-'}</Link>
                  </div>
              </div>
              }
            </div>
