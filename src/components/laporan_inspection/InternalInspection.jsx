...
import { addInternalInspection, deleteInternalInspection, getInternalInspectionByLaporanInspection, updateInternalInspection } from "../../services/internal_inspection.service";
...
const InternalInspection = () => {
  const { id } = useParams();
  const [InternalInspection, setInternalInspection] = useState([]);
  ...
  useEffect(() => {
    fetchInternalInspection();
    fetchHistoricalMemorandum();
  }, []);
  

  const fetchInternalInspection = async () => {
    try {
      setLoading(true);
      const data = await getInternalInspectionByLaporanInspection(id);
      setInternalInspection(data.data);
      console.log("Internal Inspection Data:", data.data);
      
      // Fetch data logic here
      // setInternalInspection(data);
    } catch (error) {
      console.error("Error fetching internal inspection data:", error);
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleAddInternalInspection = async (e) => {
    e.preventDefault();
    try {
      ...
      const data = await addInternalInspection(formData);
      console.log("Added Internal Inspection:", data);
      handleClose();
      fetchInternalInspection();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Internal Inspection berhasil ditambahkan!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error adding internal inspection:", error);
  ...
  const handleUpdateInternalInspection = async (e) => {
    ...
    try {
      ...
      formData.append('laporan_inspection_id', id);
      const data = await updateInternalInspection(SelectedEditData.id, formData);
      console.log("updated Internal Inspection:", data);
      handleClose();
      fetchInternalInspection();
      Swal.fire({
        title: 'Berhasil!',
        text: 'Internal Inspection berhasil diupdate!',
        icon: 'success',
        showConfirmButton: true,
        timer: 1500
      });
    } catch (error) {
      console.error("Error updating internal inspection:", error);
      setValidation(error.response?.data.errors || []);
    } finally {
      setIsSubmitting(false);
    }
      ...
      <div className='flex py-2 pl-5 '>
        {params.row.laporan_file ? (
          <Tooltip title={`${params.row.laporan_file}`} placement='bottom'>
            <Link
              to={`${base_public_url}laporan_inspection/internal_inspection/${params.value}`}
              target='_blank'
              className='text-lime-400 px-2 rounded-md hover:underline cursor-pointer'
  ...
  const handleDelete = async (row) => {
      const result = await Swal.fire({
          ...
          text: "Data Internal Inspection akan dihapus secara permanen!",
          icon: "warning",
      ...
      if (result.isConfirmed) {
          try {
              const res = await deleteInternalInspection(row.id);
              if (res.success) {
                  Swal.fire("Berhasil!", "Internal Inspection berhasil dihapus!", "success");
                  fetchInternalInspection();
              } else {
                  Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Internal Inspection!", "error");
              }
          } catch (error) {
              console.error("Error deleting Internal Inspection:", error);
              Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Internal Inspection!", "error");
          }
      }
  };
  ...
  return (
    <div className="flex flex-col space-y-4">
      <div className="flex flex-row justify-between space-x-4">
        <h1 className="text-xl font-bold p-2 uppercase">Internal Inspection</h1>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleAddInternalInspection(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
              Tambah Internal Inspection
            </h1>
      ...
      >
        <Box className="bg-white rounded-2xl shadow-lg p-4 relative top-1/2 left-1/2 w-[90%] md:w-1/3 transform -translate-x-1/2 -translate-y-1/2 ">
          <form onSubmit={(e) => handleUpdateInternalInspection(e)} method="POST" className="space-y-4">
            <h1 className="text-xl uppercase text-gray-900 mb-4">
              ...
              <div>
                ...
                <input 
                  ...
                  <div className="text-sm bg-lime-300 text-emerald-950 p-2 rounded-xl">
                    <Link className="hover:underline" target="_blank" onClick={() => handleAddActivity(SelectedEditData?.laporan_file, "LAPORAN INSPEKSI") } to={`${base_public_url}laporan_inspection/internal_inspection/${SelectedEditData.laporan_file}`}>{SelectedEditData.laporan_file ? SelectedEditData.laporan_file : '-'}</Link>
                  </div>
              </div>
              }
            </div>
