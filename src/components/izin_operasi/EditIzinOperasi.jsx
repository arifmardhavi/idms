...
} from '../../services/izin_operasi.service';
...
const EditIzinOperasi = () => {
  ...
  const fetchIzinOperasiById = async () => {
    setIsLoading(true);
    try {
      const data = await getIzinOperasiById(id);
      const izinOperasiData = data?.data || {};
      setIzinOperasi(izinOperasiData);
      setIsRLA(!!izinOperasiData.rla);
      setIsReEng(!!izinOperasiData.re_engineer);
      setSelectedTagNumber(izinOperasiData?.tag_number_id || '');
      setTagnumberId(izinOperasiData?.tag_number_id || '');
      setUnitId(izinOperasiData?.plo?.unit_id || '');
      setSelectedPlo(izinOperasiData?.plo?.id || '');
    } catch (error) {
      console.error("Error fetching Izin Operasi:", error);
    } finally {
      setIsLoading(false);
    }
  };
                    ...
                    <div className='w-full'>
                      <div className='mb-2'>
                        ...
                        <input
                          ...
                          className='w-full px-3 py-2 border border-gray-300 rounded-lg'
                        />
                        {validation.izin_operasi_certificate && (
                          validation.izin_operasi_certificate.map((item, index) => (
                            <div key={index}>
                              <small className="text-red-600 text-sm">{item}</small>
                            </div>
                          ))
                        ...
                        {izinOperasi.izin_operasi_certificate ? (
                          <>
                            <Link
                              ...
                              onClick={() => handleAddActivity(izinOperasi.izin_operasi_certificate, "Izin Operasi")}
                            >
                              {izinOperasi.izin_operasi_certificate}
                            </Link>
                            <IconX
                        ...
                        {izinOperasi.izin_operasi_old_certificate ? (
                          <>
                            <Link
                              ...
                              target='_blank'
                              className='text-emerald-950 hover:underline cursor-pointer'
                              onClick={() => handleAddActivity(izinOperasi.izin_operasi_old_certificate, "Izin Operasi")}
                            >
                              {izinOperasi.izin_operasi_old_certificate}
                            </Link>
                            <IconX
                    ...
                    <div className='flex flex-row space-x-2 w-full'>
                      ...
                      {IsReEng && (
                        <div className='w-full'>
                          ...
                          <div className='flex flex-row justify-between items-center w-full border bg-lime-400 rounded p-1'>
                            {izinOperasi.re_engineer_certificate ? (
                              <>
                                <Link
                                  to={`${base_public_url}izin_operasi/re_engineer/${izinOperasi.re_engineer_certificate}`}
                                  target='_blank'
                                  className='text-emerald-950 hover:underline cursor-pointer'
                                  onClick={() => handleAddActivity(izinOperasi.re_engineer_certificate, "Izin Operasi")}
                                >
                                  {izinOperasi.re_engineer_certificate}
                                </Link>
                                <IconX
                                  onClick={() =>
                                    handledeleteFile({
                                      re_engineer_certificate: izinOperasi.rla_certificate,
                                    })
                                  }
                                  className='text-red-500 cursor-pointer hover:rotate-90 transition duration-500 '
                                />
                            ...
                            {izinOperasi.rla_certificate ? (
                              <>
                                <Link
                                  ...
                                  target='_blank'
                                  className='text-emerald-950 hover:underline cursor-pointer'
                                  onClick={() => handleAddActivity(izinOperasi.rla_certificate, "Izin Operasi")}
                                >
                                  {izinOperasi.rla_certificate}
                                </Link>
                                <IconX
                            ...
                            {izinOperasi.rla_old_certificate ? (
                              <>
                                <Link
                                  ...
                                  target='_blank'
                                  className='text-emerald-950 hover:underline cursor-pointer'
                                  onClick={() => handleAddActivity(izinOperasi.rla_old_certificate, "Izin Operasi")}
                                >
                                  {izinOperasi.rla_old_certificate}
                                </Link>
                                <IconX
