...
const EditIzinDisnaker = () => {
  ...
  const fetchIzinDisnakerById = async () => {
    setIsLoading(true);
    try {
      const data = await getIzinDisnakerById(id);
      const izinDisnakerData = data?.data || {};
      setIzinDisnaker(izinDisnakerData);
      setIsRLA(!!izinDisnakerData.rla);
      setIsReEng(!!izinDisnakerData.re_engineer);
      setSelectedTagNumber(izinDisnakerData?.tag_number_id || '');
      setTagnumberId(izinDisnakerData?.tag_number_id || '');
      setUnitId(izinDisnakerData?.plo?.unit_id || '');
      setSelectedPlo(izinDisnakerData?.plo?.id || '');
    } catch (error) {
                    ...
                    <div className='w-full'>
                      <div className='mb-2'>
                        ...
                        <input
                          ...
                          className='w-full px-3 py-2 border border-gray-300 rounded-lg'
                        />
                        {validation.izin_disnaker_certificate && (
                          validation.izin_disnaker_certificate.map((item, index) => (
                            <div key={index}>
                              <small className="text-red-600 text-sm">{item}</small>
                            </div>
                          ))
                        ...
                        {izinDisnaker.izin_disnaker_certificate ? (
                          <>
                            <Link
                              ...
                              onClick={() => handleAddActivity(izinDisnaker.izin_disnaker_certificate, "Izin Disnaker")}
                            >
                              {izinDisnaker.izin_disnaker_certificate}
                            </Link>
                            <IconX
                        ...
                        {izinDisnaker.izin_disnaker_old_certificate ? (
                          <>
                            <Link
                              ...
                              target='_blank'
                              className='text-emerald-950 hover:underline cursor-pointer'
                              onClick={() => handleAddActivity(izinDisnaker.izin_disnaker_old_certificate, "Izin Disnaker")}
                            >
                              {izinDisnaker.izin_disnaker_old_certificate}
                            </Link>
                            <IconX
                    ...
                    <div className='flex flex-row space-x-2 w-full'>
                      ...
                      {IsReEng && (
                        <div className='w-full'>
                          ...
                          <div className='flex flex-row justify-between items-center w-full border bg-lime-400 rounded p-1'>
                            {izinDisnaker.re_engineer_certificate ? (
                              <>
                                <Link
                                  to={`${base_public_url}izin_disnaker/re_engineer/${izinDisnaker.re_engineer_certificate}`}
                                  target='_blank'
                                  className='text-emerald-950 hover:underline cursor-pointer'
                                  onClick={() => handleAddActivity(izinDisnaker.re_engineer_certificate, "Izin Disnaker")}
                                >
                                  {izinDisnaker.re_engineer_certificate}
                                </Link>
                                <IconX
                                  onClick={() =>
                                    handledeleteFile({
                                      re_engineer_certificate: izinDisnaker.rla_certificate,
                                    })
                                  }
                                  className='text-red-500 cursor-pointer hover:rotate-90 transition duration-500 '
                                />
                            ...
                            {izinDisnaker.rla_certificate ? (
                              <>
                                <Link
                                  ...
                                  target='_blank'
                                  className='text-emerald-950 hover:underline cursor-pointer'
                                  onClick={() => handleAddActivity(izinDisnaker.rla_certificate, "Izin Disnaker")}
                                >
                                  {izinDisnaker.rla_certificate}
                                </Link>
                                <IconX
                            ...
                            {izinDisnaker.rla_old_certificate ? (
                              <>
                                <Link
                                  ...
                                  target='_blank'
                                  className='text-emerald-950 hover:underline cursor-pointer'
                                  onClick={() => handleAddActivity(izinDisnaker.rla_old_certificate, "Izin Disnaker")}
                                >
                                  {izinDisnaker.rla_old_certificate}
                                </Link>
                                <IconX
