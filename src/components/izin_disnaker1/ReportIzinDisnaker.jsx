...
import { addReport, deleteReport, getReportByIzinDisnaker } from "../../services/report_izin_disnaker.service"
...
const ReportIzinDisnaker = () => {
  ...
  const fetchReport = async () => {
    try {
      setLoading(true);
      const data = await getReportByIzinDisnaker(id);
      setReport(data.data);
    } catch (error) {
      console.error("Error fetching REPORT Izin Disnaker:", error);
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('izin_disnaker_id', id);
      ...
      console.error("Error adding REPORT Izin Disnaker:", error);
    } finally {
  ...
  const handleDelete = async (row) => {
      const result = await Swal.fire({
        ...
        text: "File Report Izin Disnaker akan dihapus secara permanen!",
        icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res.success) {
            Swal.fire("Berhasil!", "Report Izin Disnaker berhasil dihapus!", "success");
            fetchReport();
          } else {
            Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Report Izin Disnaker!", "error");
          }
        } catch (error) {
          console.log(error);
          Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Report Izin Disnaker!", "error");
  ...
  const columns = [
    { field: 'izin_disnaker', headerName: 'Izin Disnaker',  valueGetter: (params) => params.unit.unit_name ,renderCell: (params) => <div className="py-4">
      {params.value}
    </div> },
    { field: 'report_izin_disnaker', headerName: 'Report/BAPK', width:400, renderCell: (params) => <div className="py-4">
      <Link
        to={`${base_public_url}izin_disnaker/reports/${params.row.report_izin_disnaker}`}
        target='_blank'
        className='text-lime-500 underline'
        onClick={() => handleAddActivity(params.row.report_izin_disnaker, "IzinDisnaker")}
      >
        {params.value}
      </Link>
    </div> },
                ...
                >
                  ...
                  <form method="post" encType="multipart/form-data" onSubmit={handleSubmit}>
                    <div className="m-2">
                      ...
                      <input
                        type="file"
                        id="report_izin_disnaker"
                        name="report_izin_disnaker"
...
export default ReportIzinDisnaker