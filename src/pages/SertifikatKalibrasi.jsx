...
import {
  deleteSertifikatKalibrasi,
  getSertifikatKalibrasi,
  downloadSelectedSertifikatKalibrasi,
...
const SertifikatKalibrasi = () => {
  const [sertifikat_kalibrasi, setsertifikat_kalibrasi] = useState([]);
  ...
  useEffect(() => {
    ...
    setUserLevel(level);
    fetchsertifikat_kalibrasi();
  }, []);

  const fetchsertifikat_kalibrasi = async () => {
    try {
      setLoading(true);
      const data = await getSertifikatKalibrasi();
      setsertifikat_kalibrasi(data.data);
    } catch (error) {
      console.error("Error fetching sertifikat_kalibrasi:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (row) => {
    const result = await Swal.fire({
      ...
      text: "Data sertifikat_kalibrasi akan dihapus secara permanen!",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const res = await deleteSertifikatKalibrasi(row.id);
        if (res.success) {
          Swal.fire("Berhasil!", "sertifikat_kalibrasi berhasil dihapus!", "success");
          fetchsertifikat_kalibrasi();
        } else {
          Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus sertifikat_kalibrasi!", "error");
        }
      } catch (error) {
        console.error("Error deleting sertifikat_kalibrasi:", error);
        Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus sertifikat_kalibrasi!", "error");
      }
    }
  };
  ...
  const handleDownloadSelected = () => {
    ...
    downloadSelectedSertifikatKalibrasi(selectedRows);
    Swal.fire("Berhasil!", `${selectedRows.length} file berhasil didownload!`, "success");
  };

  const handleExportToExcel = async () => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('sertifikat kalibrasi');

    worksheet.columns = [
      { header: 'Tag Number', key: 'tag_number', width: 18 },
      { header: 'No sertifikat kalibrasi', key: 'no_sertifikat_kalibrasi', width: 32 },
      ...
      { header: 'FILE sertifikat_kalibrasi', key: 'file_sertifikat_kalibrasi', width: 32 },
      { header: 'OLD FILE sertifikat_kalibrasi', key: 'file_old_sertifikat_kalibrasi', width: 32 },
    ];

    console.log(sertifikat_kalibrasi);
    sertifikat_kalibrasi.forEach((item) => {
      worksheet.addRow({
        ...
        file_sertifikat_kalibrasi: item.file_sertifikat_kalibrasi || '',
        file_old_sertifikat_kalibrasi: item.file_old_sertifikat_kalibrasi || '',
      });

      const lastRow = worksheet.lastRow;

      const no_sertifikat_kalibrasi = lastRow.getCell('no_sertifikat_kalibrasi');
      const file_sertifikat_kalibrasi = lastRow.getCell('file_sertifikat_kalibrasi');
      const file_old_sertifikat_kalibrasi = lastRow.getCell('file_old_sertifikat_kalibrasi');

      // No sertifikat_kalibrasi
      if (item.no_sertifikat_kalibrasi && item.file_sertifikat_kalibrasi) {
        no_sertifikat_kalibrasi.value = {
          text: item.no_sertifikat_kalibrasi,
          hyperlink: `${api_public}sertifikat_kalibrasi/${item.file_sertifikat_kalibrasi}`,
        };
        no_sertifikat_kalibrasi.font = {
          color: { argb: 'FF0000FF' },
          underline: true,
        };
      }

      // file sertifikat_kalibrasi
      if (item.file_sertifikat_kalibrasi) {
        file_sertifikat_kalibrasi.value = {
          text: item.file_sertifikat_kalibrasi,
          hyperlink: `${api_public}sertifikat_kalibrasi/${item.file_sertifikat_kalibrasi}`,
        };
        file_sertifikat_kalibrasi.font = {
          color: { argb: 'FF0000FF' },
          underline: true,
        };
      }
      ...
      if (item.file_old_sertifikat_kalibrasi) {
        file_old_sertifikat_kalibrasi.value = {
          text: item.file_old_sertifikat_kalibrasi,
          hyperlink: `${api_public}sertifikat_kalibrasi/${item.file_old_sertifikat_kalibrasi}`,
        };
        file_old_sertifikat_kalibrasi.font = {
          color: { argb: 'FF0000FF' },
          underline: true,
        };
      }
    ...
    saveAs(blob, `sertifikat_kalibrasi-export-${getWITDateLong()}.xlsx`);
  };
      ...
      renderCell: (params) => (
        <div className='py-4'>
          <Link
            ...
            onClick={() => handleAddActivity(params.row.file_sertifikat_kalibrasi, "sertifikat_kalibrasi")}
          >
            {params.value}
          </Link>
        </div>
        ...
        <div className='py-4 pl-4'>
          {params.row.file_old_sertifikat_kalibrasi ? (
            <Link
              ...
              onClick={() => handleAddActivity(params.row.file_old_sertifikat_kalibrasi, "sertifikat_kalibrasi")}
            >
              <IconCloudDownload stroke={2} />
            </Link>
          ) : (
    ...
    ...(userLevel !== '4' && userLevel !== '5' ? [{
      ...
      renderCell: (params) => (
        <div className='flex flex-row justify-center py-2 items-center space-x-2'>
          <Link to={`/sertifikat_kalibrasi/edit/${params.row.id}`}>
            <motion.button
      ...
      <div className={`flex flex-col ${hide ? '' : 'md:pl-64'} w-full px-2 py-4 space-y-3`}>
        ...
        <div className='w-full bg-white shadow-sm px-2 py-4 rounded-lg space-y-2'>
          <div className='flex flex-row justify-between'>
            <h1 className='text-xl font-bold uppercase'>Sertifikat Kalibrasi</h1>
            <Link
              to={'/sertifikat_kalibrasi/dashboard'}
              className='flex space-x-1 items-center px-2 py-1 bg-emerald-950 text-lime-300 text-sm rounded'
            >
              Dashboard Sertifikat Kalibrasi
            </Link>
            <div className='flex flex-row justify-end items-center space-x-2'>
              ...
              {selectedRows.length > 0 && (
                ...
                >
                  <IconCloudDownload />
                  <span>Download file sertifikat</span>
                </motion.button>
              )}
              <motion.a
                href='/sertifikat_kalibrasi'
                whileTap={{ scale: 0.9 }}
                whileHover={{ scale: 1.1 }}
                className='flex space-x-1 items-center px-2 py-1 bg-emerald-950 text-lime-300 text-sm rounded'
              >
              ...
              { userLevel !== '4' && userLevel !== '5' && <Link
                to='/sertifikat_kalibrasi/tambah'
...
export default SertifikatKalibrasi;
