...
import { deleteMoc, getMoc } from "../services/moc.service"
...
const Moc = () => {
    ...
    const handleDelete = async (row) => {
        const result = await Swal.fire({
          ...
          text: "Data Moc akan dihapus secara permanen!",
          icon: "warning",
        ...
        if (result.isConfirmed) {
            try {
                const res = await deleteMoc(row.id);
                if (res.success) {
                    Swal.fire("Berhasil!", "Moc berhasil dihapus!", "success");
                    fetchmoc();
                } else {
                    Swal.fire("Gagal!", "Terjadi kesalahan saat menghapus Moc!", "error");
                }
            } catch (error) {
                console.error("Error deleting Moc:", error);
                Swal.fire("Gagal!", "Terjadi kesalahan saat Moc!", "error");
            }
        }
    };
    const columns = [
        ...
        { field: 'no_dokumen', headerName: 'No Dokumen', width: 200,  renderCell: (params) => <div className="py-4"><Link className="underline font-semibold text-lime-800" to={`${base_public_url}moc/${params.row.moc_file}`} target="_blank">{params.value}</Link></div> },
        ...
        { 
            ...
            renderCell: (params) => (
              <div className={`bg-lime-300 text-emerald-950 my-2 p-2 rounded flex flex-col justify-center items-center`}>
                {params.row.tipe_moc == '0' ? 'Rekomendasi Rutin' : params.row.tipe_moc == '1' ? 'Rekomendasi TA' : params.row.tipe_moc == '2' ? 'Rekomendasi Overhaul' : params.row.tipe_moc == '3' ? 'Dokumen Kajian/Evaluasi' : params.row.tipe_moc == '4' ? 'Permintaan Tools' : 'Dokumen Kantor Pusat'}
              </div>
            )
        },
        {
        ...
        },
        {
          ...
          renderCell: (params) => (
            <div className='py-4 flex flex-row justify-center items-center'>
              <Link
                to={`${base_public_url}moc/${params.row.moc_file}`}
                target='_blank'
        ...
        ...(userLevel !== '4' && userLevel !== '5' ? [{
            ...
            renderCell: (params) => (
                <div className='flex flex-row justify-center py-2 items-center space-x-2'>
                  <Link to={`/moc/edit/${params.row.id}`}>
                      <button
        ...
        <div className={`flex flex-col ${hide ? '' : 'md:pl-64'} w-full px-2 py-4 space-y-3`}>
            ...
            <div className='w-full bg-white shadow-sm px-2 py-4 rounded-lg space-y-2'>
                <div className='flex flex-row justify-between'>
                    <h1 className='text-xl font-bold uppercase'>Moc</h1>
